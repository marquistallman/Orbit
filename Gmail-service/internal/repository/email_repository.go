...
type EmailRepository interface {
	...
	DeleteEmailsByUserID(userID string) (int64, error)
}
...
func (r *PostgresEmailRepository) GetOAuthTokens(userID string) (string, string, error) {
	...
	// Filter by Google provider to ensure we get Gmail-compatible tokens
	query := `
		SELECT uoa.access_token, uoa.refresh_token
		FROM user_oauth_accounts uoa
		JOIN oauth_providers op ON op.id = uoa.provider_id
		WHERE uoa.user_id = $1 AND LOWER(op.name) = 'google'
		LIMIT 1`

	err := r.DB.QueryRow(query, userID).Scan(&accessToken, &refreshToken)
	if err != nil {
		// Fallback: try without provider filter
		queryFallback := `SELECT access_token, refresh_token FROM user_oauth_accounts WHERE user_id = $1 LIMIT 1`
		err2 := r.DB.QueryRow(queryFallback, userID).Scan(&accessToken, &refreshToken)
		if err2 != nil {
...
func (r *PostgresEmailRepository) DeleteEmailsByUserID(userID string) (int64, error) {
	res, err := r.DB.Exec(`DELETE FROM emails WHERE user_id = $1`, userID)
	if err != nil {
		return 0, fmt.Errorf("error deleting emails: %w", err)
	}
